...
function AvatarClassroomTab({ state, persist, classId, COLORS, onAward }) {
  ...
  // Gender selector for class
  const [genderConfig, setGenderConfig] = React.useState(state.classGenderConfig?.[classId]||'mixed');
  function applyGender(g) {
    setGenderConfig(g);
    const newAvatars = { ...mergedAvatars };
    students.forEach(s => {
      const gender = g==='girls'?'female':g==='boys'?'male':Math.random()>0.5?'male':'female';
      newAvatars[s.id] = generateAvatar(s.id+'_'+g, gender);
    });
    try { localStorage.setItem('najm_avatars', JSON.stringify(newAvatars)); } catch {}
    setLocalAvatars(newAvatars);
    persist(prev => ({ ...prev, studentAvatars: newAvatars, classGenderConfig: { ...prev.classGenderConfig, [classId]: g } }));
  }
  ...
  return (
    <div>
      ...
      <div style={{display:'flex',gap:8,marginBottom:20,flexWrap:'wrap',alignItems:'center'}}>
        <span style={{fontSize:12,fontWeight:'bold',color:COLORS.textMuted}}>Class gender:</span>
        {[{v:'boys',l:'👦 All Boys'},{v:'girls',l:'👧 All Girls'},{v:'mixed',l:'👦👧 Mixed'}].map(opt=>(
          <button key={opt.v} onClick={()=>applyGender(opt.v)}
            style={{fontSize:12,padding:'6px 14px',borderRadius:8,border:`2px solid ${genderConfig===opt.v?COLORS.robotics:COLORS.border}`,background:genderConfig===opt.v?`${COLORS.robotics}18`:'white',cursor:'pointer',fontWeight:'bold',color:genderConfig===opt.v?COLORS.robotics:COLORS.textMuted}}>
            {opt.l}
          </button>
        ))}
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill, minmax(120px, 1fr))',gap:16}}>
        {students.map(student => {
          ...
          const av = mergedAvatars[student.id];
          return (
            <div key={student.id}
              style={{background:COLORS.panel,border:`1px solid ${COLORS.border}`,borderRadius:16,padding:14,textAlign:'center',cursor:'pointer',transition:'all 0.2s'}}
              onMouseEnter={e=>e.currentTarget.style.boxShadow=`0 6px 20px ${COLORS.robotics}33`}
              onMouseLeave={e=>e.currentTarget.style.boxShadow='none'}
            >
              <StudentAvatar student={{...student, _av: av}} size="md" clickable onAward={handleAward} showName={false}
                state={{...state, studentAvatars: mergedAvatars}} COLORS={COLORS} />
              <div style={{fontWeight:'bold',fontSize:13,marginTop:8,color:COLORS.text}}>{student.name}</div>
              <div style={{fontSize:11,color:COLORS.xp,fontWeight:'bold',marginTop:2}}>{xp} XP</div>
...
function StudentReportGenerator({ state, persist, COLORS }) {
  const [selectedStudentId, setSelectedStudentId] = React.useState(state.students[0]?.id || '');
  const [positiveAttitudes, setPositiveAttitudes] = React.useState([]);
  ...
  return (
    <div>
      <div style={{marginBottom:20}}>
        <h2 style={{fontSize:20,fontWeight:'bold',color:COLORS.text,margin:'0 0 4px'}}>📝 AI Student Reports</h2>
        <p style={{margin:0,fontSize:12,color:COLORS.textMuted}}>Generate professional progress notes in seconds</p>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:16}}>
          ...
          <div style={{background:COLORS.panel,border:`1px solid ${COLORS.border}`,borderRadius:12,padding:16}}>
            <div style={{fontSize:12,fontWeight:'bold',color:COLORS.textMuted,marginBottom:8}}>SELECT STUDENT</div>
            <select value={selectedStudentId} onChange={e=>setSelectedStudentId(e.target.value)}
              style={{width:'100%',padding:'8px 12px',borderRadius:8,border:`1px solid ${COLORS.border}`,background:COLORS.panelAlt,fontSize:13,color:COLORS.text}}>
              {state.students.map(s=>(
                <option key={s.id} value={s.id}>{s.name} — {totalXP(state,s.id)} XP</option>
              ))}
            </select>
            {student && (
              <div style={{display:'flex',alignItems:'center',gap:10,marginTop:10,padding:10,background:COLORS.panelAlt,borderRadius:8}}>
                <div style={{width:40,height:40,borderRadius:'50%',overflow:'hidden',border:`2px solid ${COLORS.robotics}`,background:'white'}}>
                  {state.studentAvatars?.[student.id]
                    ? <AvatarSVG av={state.studentAvatars[student.id]} size={36}/>
                    : <span style={{fontSize:20,lineHeight:'36px',display:'block',textAlign:'center'}}>{student.name[0]}</span>}
                </div>
                <div>
                  <div style={{fontWeight:'bold',fontSize:13}}>{student.name}</div>
                  <div style={{fontSize:11,color:COLORS.xp}}>{xp} XP earned</div>
                </div>
              </div>
            )}
          </div>
          ...
          {/* Positive attitudes */}
          <div style={{background:COLORS.panel,border:`1px solid ${COLORS.border}`,borderRadius:12,padding:16}}>
            <div style={{fontSize:12,fontWeight:'bold',color:COLORS.behavior,marginBottom:8}}>✅ POSITIVE ATTITUDES</div>
            <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
              {POSITIVE.map(item=>(
                <button key={item} onClick={()=>toggleItem(positiveAttitudes,setPositiveAttitudes,item)}
                  style={{fontSize:11,padding:'4px 10px',borderRadius:20,cursor:'pointer',fontWeight:'bold',border:`1px solid ${positiveAttitudes.includes(item)?COLORS.behavior:COLORS.border}`,background:positiveAttitudes.includes(item)?`${COLORS.behavior}18`:'white',color:positiveAttitudes.includes(item)?COLORS.behavior:COLORS.textMuted}}>
                  {item}
                </button>
              ))}
            </div>
          ...
          <div style={{background:COLORS.panel,border:`1px solid ${COLORS.border}`,borderRadius:12,padding:16}}>
            <div style={{fontSize:12,fontWeight:'bold',color:COLORS.challenge,marginBottom:8}}>⚠️ AREAS FOR IMPROVEMENT</div>
            <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
              {AREAS.map(item=>(
                <button key={item} onClick={()=>toggleItem(areasForImprovement,setAreasForImprovement,item)}
                  style={{fontSize:11,padding:'4px 10px',borderRadius:20,cursor:'pointer',fontWeight:'bold',border:`1px solid ${areasForImprovement.includes(item)?COLORS.challenge:COLORS.border}`,background:areasForImprovement.includes(item)?`${COLORS.challenge}18`:'white',color:areasForImprovement.includes(item)?COLORS.challenge:COLORS.textMuted}}>
                  {item}
                </button>
              ))}
            </div>
          ...
          <button onClick={generateReport} disabled={loading||!selectedStudentId}
            style={{padding:'12px',borderRadius:10,border:'none',background:loading?COLORS.panelAlt:COLORS.robotics,color:loading?COLORS.textMuted:'white',fontWeight:'bold',fontSize:14,cursor:loading?'not-allowed':'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:8}}>
            {loading ? '⏳ Generating...' : '✨ Generate AI Report'}
          </button>
        </div>

        {/* Right: Output */}
        <div style={{background:COLORS.panel,border:`1px solid ${COLORS.border}`,borderRadius:12,padding:20,display:'flex',flexDirection:'column',gap:12}}>
          <div style={{fontSize:12,fontWeight:'bold',color:COLORS.textMuted}}>GENERATED REPORT</div>
          {!generatedReport && !loading && (
            <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',color:COLORS.textFaint,fontSize:13,textAlign:'center',padding:40}}>
              Select a student and click Generate to create a progress note ✨
            </div>
          )}
          {loading && (
          ...
          {generatedReport && (
            <>
              <div style={{flex:1,padding:16,background:COLORS.panelAlt,borderRadius:10,fontSize:14,lineHeight:1.7,color:COLORS.text,fontStyle:'italic'}}>
                "{generatedReport}"
              </div>
              <button onClick={copyReport}
                style={{padding:'10px',borderRadius:8,border:`1px solid ${COLORS.border}`,background:copied?COLORS.behavior:'white',color:copied?'white':COLORS.text,fontWeight:'bold',fontSize:13,cursor:'pointer'}}>
